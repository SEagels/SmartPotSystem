...
def setup_mqtt_handlers():
    mqtt_manager.on_message("smartpot/+/telemetry/sensors", _handle_telemetry)
    mqtt_manager.on_message("smartpot/+/event/device_status", _handle_device_status)
    ...
    mqtt_manager.subscribe("smartpot/+/telemetry/sensors", qos=1)
    mqtt_manager.subscribe("smartpot/+/event/device_status", qos=1)
    mqtt_manager.subscribe("smartpot/+/event/watering", qos=1)
    mqtt_manager.subscribe("smartpot/+/response/+", qos=1)
...
async def _handle_telemetry(topic: str, payload: dict):
    ...
    try:
        ...
        if user_id:
            ...
            if payload.get("water_tank_level_pct") is not None and payload["water_tank_level_pct"] < 20:
                from app.services.alert_service import create_alert
                async with sessionmaker() as db2:
                    await create_alert(
                        db2, device_id, device.user_id, "water_low", "warning",
                        "水箱余量不足",
                        f"当前水箱余量仅{payload['water_tank_level_pct']:.0f}%，请及时加水",
                    )
                    await db2.commit()
    except Exception:
