...
name_pretrain = os.path.join(root, "yolo11n.pt")
## 原始训练路径
...
def step1_train():
    ...
    model.train(data=name_yaml, device="0", imgsz=640, epochs=50, batch=64, workers=16, save_period=1)  # train the model


## 2024.3.4添加【amp=False】
def step2_Constraint_train():
    model = YOLO(name_train)
    model.train(data=name_yaml, device="0", imgsz=640, epochs=50, batch=32, amp=False, workers=16, save_period=1,
...
def step4_finetune():
    ...
    model.train(data=name_yaml, device="0", imgsz=640, epochs=200, batch=64, workers=16, name=path_fineturn)  # train the model
...
def step5_distillation():
    ...
    model_s.train(data="data.yaml", Distillation=model_t.model, loss_type='mgd',layers=layers, amp=False, imgsz=1280, epochs=300,
                  batch=64, device=0, workers=16, lr0=0.001)
